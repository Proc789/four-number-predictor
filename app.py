# app-hotboost-v5-rhythm（觀察期更新節奏 + 命中統計 + 保底補碼）
...
<!DOCTYPE html>
<html>
<head><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1'><title>預測器 - 追關版</title></head>
...
<br><br>
{% if prediction %}<div><strong>本期預測號碼：</strong> {{ prediction }}（目前第 {{ stage }} 關 / 建議下注第 {{ bet_stage }} 關）</div>{% endif %}
{% if last_prediction %}<div><strong>上期預測號碼：</strong> {{ last_prediction }}</div>{% endif %}
{% if observation_message %}<div style='color: gray;'>{{ observation_message }}</div>{% endif %}
<div>熱號池節奏狀態：{{ rhythm_state }}</div>
{% if training %}
<div style='margin-top: 20px; text-align: left;'><strong>命中統計：</strong><br>
冠軍命中次數（任一區）：{{ all_hits }} / {{ total_tests }}<br>
熱號命中次數：{{ hot_hits }}<br>
熱號池命中次數：{{ hot_pool_hits }}<br>
動熱命中次數：{{ dynamic_hits }}<br>
補碼命中次數：{{ extra_hits }}</div>
{% endif %}
{% if history %}
<div style='margin-top: 20px; text-align: left;'><strong>最近輸入紀錄：</strong><ul>
{% for row in history[-10:] %}<li>第 {{ loop.index }} 期：{{ row }}</li>{% endfor %}</ul></div>
{% endif %}</body></html>
"""

@app.route('/observe')
def observe():
    global was_observed, observation_message, rhythm_state, rhythm_history, hot_pool, predictions
    was_observed = True
    ...
    if training_mode or len(history) >= 5:
        ...
        if len(predictions) >= 2:
            champion = history[-1][0] if history else None
...
def index():
    ...
    global rhythm_state, was_observed, observation_message, hot_pool
    ...
    if request.method == 'POST':
        try:
            ...
            if len(predictions) >= 1:
                ...
                if training_mode:
                    total_tests += 1
                    if champion in last_pred[:2]: hot_hits += 1
                    elif champion in last_pred[2:4]: dynamic_hits += 1
                    elif champion in last_pred[4:]: extra_hits += 1

                    if champion in hot_pool: hot_pool_hits += 1
            ...
            prediction = generate_prediction(current_stage if 1 <= current_stage <= 4 else 1)
            predictions.append(prediction)
            was_observed = False
            observation_message = ""
